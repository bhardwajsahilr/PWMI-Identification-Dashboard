...
export function PsychoSocialForm() {
  ...
  // Collect non-empty topics for display in entry cards
  const getTopics = (entry: PsychoSocialEntry) => {
    return [
    entry.topicName1,
    entry.topicName2,
    entry.topicName3,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="animate-in fade-in slide-in-from-top-4 space-y-6 border border-teal/20 rounded-xl p-6 bg-teal/5">
          ...
          <Accordion
          title="Psycho-social Education Sessions"
          icon={<BookOpen className="h-5 w-5" />}>
          
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-neutral-secondary mb-2">
                  Psycho-social education sessions attended
                </label>
                <select
